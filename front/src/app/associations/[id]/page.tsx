...
export async function generateStaticParams() {
  const shelters = await fetch(process.env.NEXT_PUBLIC_API_URL + `/associations`).then(res =>
    res.json()
  );
...
export default async function ShelterDetails({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const shelter = await fetch(process.env.NEXT_PUBLIC_API_URL + '/associations/' + Number(id)).then(
    res => res.json()
  );
        ...
        <div className="font-body mx-auto w-[80%] rounded-lg my-1 justify-center flex">
          {shelterUrl ? (
            <img
              ...
              src={`${process.env.NEXT_PUBLIC_API_URL}` + `${shelterUrl}`}
