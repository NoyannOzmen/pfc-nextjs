...
const ShelterCard = ({ shelter }: ShelterCardProps) => {
  ...
  <div className="bg-zoning rounded-lg shadow dark:bg-gray-800 flex flex-col">
    <div className="relative md:w-full flex justify-center items-center">
      { url ? (
        <img className="font-body rounded-lg" src={`${process.env.NEXT_PUBLIC_API_URL}` + `${url}`} alt={`Logo de ${shelter.nom}`} />
      ) : (
    ...
    <div className="flex text-center flex-col">
      ...
      <div className="flex text-sm font-medium justify-center items-end">
        <Link className="my-2 bg-accents1-light text-fond w-[90%] transition ease-in duration-200 text-center text-xs md:text-2xl font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg" href={`/associations/${shelter.id}`}>En savoir plus</Link>
      </div>
    </div>
  </div>
    )
